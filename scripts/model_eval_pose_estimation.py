...
if __name__ == "__main__":
    ...
    image_dir = "C:\\Users\\jlim\\OneDrive - Cor Medical Ventures\\Documents\\Channel Robotics\\Catheter Calibration Data\\LC_v3_05_20_25_T1\\image_snapshots_eval"

    print("Loading camera calibration data...")
    # Load pixel color classification model
    with open("pixel_classification/rf_2class_model.pkl", "rb") as f:
    ...
    for img_num, (image0, image1) in enumerate(
        ...
        for cam_num, image in enumerate([img0_cropped, img1_cropped]):
            image_hsv = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)
            image_reshaped = image_hsv.reshape((-1, 3))
            # Predict mask using pixel classifier
            color_mask = pixel_classifier.predict(image_reshaped)
            # Reshape color_mask to the original image dimensions and display it
            color_mask_img = color_mask.reshape(image.shape[0], image.shape[1])
            # plt.figure(figsize=(6, 6))
            # plt.imshow(color_mask_img, cmap="jet")
            # plt.title("Color Mask")
            # plt.axis("off")
            # plt.show()

            fg_mask = color_mask == 1
            # Centroid of foreground mask
            fg_centroid = np.mean(
                np.argwhere(fg_mask.reshape(image.shape[0], image.shape[1])),
                axis=0,
            ).astype(int)

            foreground_prob = pixel_classifier.predict_proba(image_reshaped)[
                :, 1
            ]
            max_idx_fg = np.argmax(foreground_prob)
            max_y_fg, max_x_fg = np.unravel_index(
                max_idx_fg, (image.shape[0], image.shape[1])
            )
            background_prob = pixel_classifier.predict_proba(image_reshaped)[
                :, 0
            ]
            max_idx_bg = np.argmax(background_prob)
            max_y_bg, max_x_bg = np.unravel_index(
                max_idx_bg, (image.shape[0], image.shape[1])
            )
            input_point = np.array(
                [[fg_centroid[1], fg_centroid[0]], [max_x_bg, max_y_bg]]
            )
            ...
            # Pad masks to match original image size
            box = box0 if cam_num == 0 else box1
            mask = np.pad(
                masks[0],
                (
                    (box[1], img0.shape[0] - box[3]),
                    (box[0], img0.shape[1] - box[2]),
                ),
                mode="constant",
                constant_values=0,
            )
            input_point_original = input_point + np.array([box0[0], box0[1]])
    ...
    tip_pose_df.to_csv(
        os.path.join(save_dir, "tip_coordinates_model_eval.csv"), index=True
    )
